...
export const AIModal: React.FC<AIModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden animate-swift-enter" onClick={!loading ? onClose : undefined}>
      ...
      {/* Compact Resizable Modal Container */}
      <div 
        className="apple-glass w-full relative z-10 flex flex-col border border-white/20 shadow-2xl transition-all duration-700
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-5 sm:p-8 space-y-6 sm:space-y-8">
          ...
          <div className="flex justify-between items-start">
             ...
             {!loading && (
               <button onClick={onClose} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/5 flex items-center justify-center text-white/20 hover:text-white transition-all hover:rotate-90 swift-press">
          ...
          {/* Error Channel */}
          {error && (
            <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 flex flex-col gap-3 animate-swift-enter shadow-inner">
              <div className="flex items-start gap-3">
                <AlertCircle size={16} className="text-red-500 shrink-0 mt-0.5" />
                <div className="flex-1">
                  <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest mb-0.5">Link_Interrupted</p>
                  <p className="text-[10px] sm:text-xs text-red-400/80 leading-relaxed italic font-medium">{error}</p>
                </div>
              </div>
              <button 
          ...
          <div className="space-y-2.5">
            ...
            <textarea 
              ...
              className="w-full bg-white/[0.02] border border-white/10 rounded-[24px] p-5 sm:p-6 text-white text-sm sm:text-base font-bold tracking-tight italic min-h-[140px] sm:min-h-[180px] resize-none focus:border-white/30 transition-all outline-none placeholder:text-white/10 shadow-inner no-scrollbar"
              placeholder={isGemini ? "Describe focus targets..." : "ChatGPT context seed..."}
              disabled={loading}
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`w-9 h-5 rounded-full relative transition-all duration-700 ease-in-out ${deepThink ? 'bg-[var(--theme-primary)] shadow-[0_0_12px_var(--theme-primary-glow)]' : 'bg-white/10'}`}>
                  <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-700 cubic-bezier(0.34, 1.56, 0.64, 1) ${deepThink ? 'translate-x-5 scale-110' : 'translate-x-1 scale-100'}`} />
                </div>
              </div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className={`w-9 h-5 rounded-full relative transition-all duration-700 ease-in-out ${isFuture ? 'bg-amber-500 shadow-[0_0_12px_rgba(245,158,11,0.4)]' : 'bg-white/10'}`}>
                  <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-700 cubic-bezier(0.34, 1.56, 0.64, 1) ${isFuture ? 'translate-x-5 scale-110' : 'translate-x-1 scale-100'}`} />
                </div>
              </div>
        ...
        <div className="p-5 sm:p-8 pt-0">
          <button 
            ...
            disabled={loading}
            className={`w-full py-4 sm:py-5 rounded-[24px] font-black text-base sm:text-lg italic uppercase tracking-[0.25em] transition-all flex items-center justify-center gap-3 relative overflow-hidden shadow-2xl ${
