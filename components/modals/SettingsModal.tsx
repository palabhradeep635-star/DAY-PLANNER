...
export const SettingsModal: React.FC<SettingsModalProps> = ({ onClose }) => {
  ...
  const SettingRow = ({ icon: Icon, label, subLabel, value, onClick, toggle = false, danger = false, rightElement }: any) => (
    <div 
      onClick={onClick}
      className={`flex items-center justify-between p-4 bg-white/5 border-b border-white/5 last:border-0 active:bg-white/10 transition-colors cursor-pointer group ${danger ? 'text-red-400' : 'text-white'}`}
    >
      <div className="flex items-center gap-4">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${danger ? 'bg-red-500/10' : 'bg-white/5'}`}>
          <Icon size={20} className={danger ? 'text-red-500' : 'theme-text'} />
        </div>
        <div>
           <div className="text-sm font-bold uppercase tracking-tight italic">{label}</div>
  ...
  if (showProviderSelect) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-enter" onClick={() => setShowProviderSelect(false)}>
        ...
        <div className="apple-glass w-full max-w-sm relative z-10 p-6 !rounded-[32px] border border-white/20 shadow-2xl" onClick={e => e.stopPropagation()}>
           ...
           <div className="space-y-3">
              {[
                ...
                { id: 'chatgpt', name: 'GPT Forge', model: 'ChatGPT Protocol', icon: Bot, color: 'text-emerald-400' }
              ].map((p) => (
                <button
                  key={p.id}
                ...
                >
                  ...
                  {settings.aiProvider === p.id && <Check size={18} className="text-white" />}
                </button>
              ))}
           </div>
           <button onClick={() => setShowProviderSelect(false)} className="w-full mt-6 py-3 text-[10px] font-black text-white/30 uppercase tracking-[0.3em]">Cancel_Handshake</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="absolute inset-0 bg-black/80 backdrop-blur-xl animate-enter" />
      <div 
        className="apple-glass w-full max-w-md relative z-10 p-0 overflow-hidden !rounded-[40px] border border-white/20 max-h-[90vh] overflow-y-auto no-scrollbar shadow-3xl" 
        onClick={e => e.stopPropagation()}
      >
        <div className="p-6 flex justify-between items-center border-b border-white/5">
          <h2 className="text-xl font-black text-white italic uppercase tracking-tight">System_Control</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white/30 hover:text-white">
            <X size={20} />
          </button>
        </div>
          ...
          <div className="space-y-6">
            <div className="rounded-[28px] overflow-hidden border border-white/5">
              <SettingRow 
                ...
                rightElement={
                  <div className="px-3 py-1 rounded-lg bg-white/5 border border-white/10 text-[9px] font-black text-white/60 uppercase">
                    Switch
                  </div>
                }
              />
              ...
              <div className="bg-white/5 p-5 border-b border-white/5">
                ...
                <div className="space-y-2">
                  ...
                  <SettingRow 
                    icon={Database} 
                    label="Notion Sync" 
                    value={settings.integrations.notion} 
                    toggle 
                    onClick={() => handleToggleIntegration('notion')} 
                  />
                  <SettingRow 
                    icon={Share2} 
                    label="Microsoft Tasks" 
                    value={settings.integrations.microsoftTasks} 
                    toggle 
                    onClick={() => handleToggleIntegration('microsoftTasks')} 
                  />
                </div>
              </div>
                  ...
                  {CHASSIS_SKINS.map((t) => (
                    <button
                      ...
                      className={`relative aspect-video rounded-xl overflow-hidden group transition-all duration-500 border-2 ${
                        settings.gradientTheme === t.id ? 'border-white ring-4 ring-white/10' : 'border-transparent hover:border-white/20'
                      }`}
                    >
                      <div className="absolute inset-0 opacity-80" style={{ background: t.gradient }} />
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-[8px] font-black text-white uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 px-2 py-1 rounded backdrop-blur-md">
                          {t.name}
                        </span>
